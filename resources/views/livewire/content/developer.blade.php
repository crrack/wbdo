            ...
            >
                ...
                >
                    <div class="p-1 -mt-2 space-y-1 text-gray-400 bg-gray-100 rounded-b-lg cursor-default draggable-mirror-hide">
                        ...
                        @include('livewire.content.developer.form')

                        @if($element['type'] == 'list' && !($state[$element['id']]['delete'] ?? false))
                            <div class="p-3" x-show="!open">
                                @include('livewire.content.developer.list')
                            </div>
                        @endif
                    </div>

                    @include('livewire.content.developer.item')
                </div>
            </div>
        @endforeach
    </div>
    @if(($state[$group]['type'] ?? null) == 'list')
        @include('livewire.content.developer.add-group')
    @else
        @include('livewire.content.developer.add-element')
    @endif
